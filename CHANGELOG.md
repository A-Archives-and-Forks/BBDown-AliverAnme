...
- **`sub check --per-sub-dir`：按订阅分目录下载**。多个订阅（多 UP 主/账号）此前全部平铺在 `-w` 根目录，无法区分归属；开启后每个订阅下载到 `<work-dir>/<订阅名>/` 子目录，订阅名取 `sub add --name` 的显示名（缺省为 target），经 `PathUtil.SanitizePathSegment` 净化（剔除路径分隔符/保留名/纯点段），净化后同名的订阅追加 `-2/-3` 序号避免互相覆盖。默认关闭，单目录行为完全不变。同时：`sub check` 的 `-w` 在首个下载前统一解析为绝对路径，修复相对 `-w` 多订阅时进程 CWD 漂移导致的 `<root>/<sub1>/<sub2>` 嵌套。
